...
func (tw *TimeWheel) addAnyUnsafe(delay time.Duration, callback func(), circle, async bool) *Task {
	if delay <= 0 {
		delay = tw.tick
	}
...
func (tw *TimeWheel) store(task *Task, circleMode bool) {
	round, index := tw.calculate(task.delay)
...
func (t *Timer) ResetBest(delays ...time.Duration) {
	...
	_, index := t.tw.calculate(delays[0])
	best := len(t.tw.buckets[index])
	bestIndex := 0
	for i, delay := range delays[1:] {
		_, index := t.tw.calculate(delay)
		if best > len(t.tw.buckets[index]) {
			best = len(t.tw.buckets[index])
			bestIndex = i
		}
	}
	delay := delays[bestIndex]
	// make new task
	var task *Task
	if t.fn != nil { // use AfterFunc
		task = t.tw.addAny(delay,
...
func (tw *TimeWheel) calculate(delay time.Duration) (round, index int) {
	delaySeconds := delay.Seconds()
	tickSeconds := tw.tick.Seconds()
	return int(delaySeconds / tickSeconds / float64(tw.bucketsNum)), (int(float64(tw.currentIndex) + delaySeconds/tickSeconds)) % tw.bucketsNum
}
